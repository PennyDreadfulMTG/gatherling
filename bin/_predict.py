...
def predict(text, limit=8, min_score=3):
    ...
    top = top or [f for f, _ in ranked[:2]]

    # Include conventional regression-test counterparts. The first Cloud pilot
    # correctly predicted model files but omitted DeckTest, EventTest, and friends,
    # making every test-first agent stop on an artificial ownership boundary.
    tests = set()
    for path in top:
        if not path.startswith('gatherling/'):
            continue
        base = path.split('/')[-1].rsplit('.', 1)[0].lower()
        tests.update(strong.get(base + 'test', ()))
    return top + sorted(tests - set(top))
