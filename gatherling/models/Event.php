...
class Event
{
    ...
    {
        return Database::list_result_single_param('SELECT player FROM entries WHERE event_id = ? ORDER BY medal, player', 's', $this->id);
    }
    ...
    public function addPlayer($playername)
    {
        ...
        if (is_null($entry)) {
            ...
            $stmt->bind_param('ss', $this->id, $player->name);
            if (!$stmt->execute()) {
                print_r($stmt->error);
