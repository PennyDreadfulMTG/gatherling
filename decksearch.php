...
function content() {
  if(!empty($_GET['deck']) || !empty($_GET['card'])) {
    ...
    if ($stmt->num_rows() == 0) { 
      ...
      echo "<tr><th>Deck ID</th><th>Deck Name</th><th>Played by</th><th>Event</th> </tr>";
      foreach ($deck_ids as $deck_id) {
        $deck = new Deck($deck_id);
        echo "<tr><td>$deck_id</td><td>" . medalImgStr($deck->medal) . "\n";
        echo $deck->linkTo();
        echo "</td>";
    ...
    echo "<input class=\"inputbox\" type=\"text\" name=\"deck\"></td></tr>";
    echo "<tr><th>Deck contains card</th><td>"; 
    echo "<input class=\"inputbox\" type=\"text\" name=\"card\"></td></tr>";
    echo "<tr><td colspan=2 class=\"buttons\">";
    echo "<input class=\"inputbutton\" type=\"submit\" value=\"Gimme some decks!\"></td></tr>";
    echo "</table></form>";
